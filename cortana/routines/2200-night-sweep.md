...
Routine id `trig_01Lq4GSrvMfwwqR3qmzSJ5Rq` · UTC cron `0 5 * * *` (Pacific 10:00pm)
...
3. CALENDARS, all seven in WIRING, today 00:00 through the day after tomorrow 24:00. ESSENTIAL. For today: which blocks happened is unknowable, so ask nothing; instead list today's ❗ items that show no ✅ and no evidence in email or the ledger, plainly and without scolding. For tomorrow: every class, appointment, DUE event and skeleton block, and every overlap between two busy events.
...
7. CONFIG lines in the doc (bCourses feed, SMS): act as described in WIRING. On the bCourses feed, also confirm every graded item inside the next 14 days has a matching DUE event on the MBA calendar.
